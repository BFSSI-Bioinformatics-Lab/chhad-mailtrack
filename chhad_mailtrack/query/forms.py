...
class QueryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'date_initial_email': forms.DateInput(attrs={'type': 'date'}),
            'date_chhad_received': forms.DateInput(attrs={'type': 'date'}),
...
class UpdateQueryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'date_initial_email': forms.DateInput(format=('%Y-%m-%d'), attrs={'type': 'date'}),
            'date_chhad_received': forms.DateInput(format=('%Y-%m-%d'), attrs={'type': 'date'}),
            'date_assigned_to_evaluator': forms.DateInput(format=('%Y-%m-%d'),
                                                          attrs={'type': 'date'}),
            'date_to_sections_head_for_approval': forms.DateInput(format=('%Y-%m-%d'),
                                                                  attrs={'type': 'date'}),
            'date_completed': forms.DateInput(format=('%Y-%m-%d'),
                                                attrs={'type': 'date'}),
        }
